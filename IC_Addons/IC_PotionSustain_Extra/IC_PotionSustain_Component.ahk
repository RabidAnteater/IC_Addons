...
Gui, ICScriptHub:Add, Checkbox, vg_PS_Checkbox_PreferSingle xs15 y+10, Prefer single potions over s+? combos to maximise Gold chests?
...
Class IC_PotionSustain_Component
{
	...
	LoadSettings()
	{
		...
		GuiControl, ICScriptHub:, g_PS_Checkbox_PreferSingle, % this.Settings["PreferSingle"]
		...
		this.PreferSingle := this.Settings["PreferSingle"]
	...
	SaveSettings()
	{
		...
		this.Settings["PreferSingle"] := g_PS_Checkbox_PreferSingle
		...
		this.PreferSingle := g_PS_Checkbox_PreferSingle
	...
	DefaultSettings()
	{
		...
		this.Settings["PreferSingle"] := false
	...
	CalculateSmallPotionSustain()
	{
		...
		else if (this.PreferSingle)
			this.SustainSmallAbility := "Modron reset of z" . rz . " allows sustaining, but preferring singles."
		else
	...
	CalculateAutomationBuffs()
	{
		...
		; Sustaining smalls.
		; Only use if modron reset is 655+ (or 475+GH) and small pots are above the minimum threshold and small pots are not disabled and prefer single potions is disabled.
		else if (this.ModronResetZone >= sZone AND this.PotAmounts["s"] > this.AutomatePotMinThresh AND !this.DisableSmall AND !this.PreferSingle)
			calcAuto := this.CalculateSustainSmalls()
		; Sustaining nothing.
		; Only use as a last resort or if preferring singles over sustaining smalls.
		else
